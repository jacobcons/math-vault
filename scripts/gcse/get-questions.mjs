...
async function main() {
  ...
  for (const id of directoryIds) {
    const res = await fetch(`https://www.drfrost.org/api/worksheets/directory/${id}?sort=date`, {
      headers: {
    ...
    for (const e of exams['_children']) {
      if (e.type === 'worksheet') {
        const res = await fetch(`https://www.drfrost.org/api/worksheets/worksheet/${e.wid}`, {
          headers: {
            accept: 'application/json, text/javascript, */*; q=0.01',
            'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8',
            baggage:
  ...
  await writeFile('./gcse.json', JSON.stringify(questionData, null, 2), 'utf-8');
}
